...
<template>
  ...
  >
    ...
    <span
      class="absolute w-px h-px p-0 -m-px overflow-hidden whitespace-nowrap clip-rect-0 border-0"
    >
      {{ props.label }}
    </span>
  </span>
...
const animationLoop = () => {
  ...
  if (
    lastPosition.value.x === mousePosition.value.x &&
    lastPosition.value.y === mousePosition.value.y
  ) {
    animationFrameId = requestAnimationFrame(animationLoop)
    return
  }
